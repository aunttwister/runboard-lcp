...
#   serve.sh vllm-cruz   vLLM + vllm-exl3 fork + turboderp 3.05bpw  [current default]
#
...
current(){
  if systemctl is-active --quiet "$VLLM_EXL3"; then echo vllm-cruz
...
case "${1:-status}" in
  ...
  vllm)
    ...
    step "rollback = serve.sh $RESTORE"
    show_status ;;
  *) echo "usage: serve.sh [status|cruz|vllm-cruz|exl3|vllm]"; exit 2 ;;
esac
